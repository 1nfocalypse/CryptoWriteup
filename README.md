...
and the message times the generator raised to the private key's power modulo $p$ (which is part of the public key) raised to the $k$th power. Decryption is handled by raising the first element of the message to the prime number minus 1 minus the generator times the content of the message. The ElGamal 
